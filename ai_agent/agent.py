...
def generate_ai_recommendation(master_status, alive_chunkservers):
    if master_status == "OFFLINE":
        return "CRITICAL: Master server is offline. Restart master.py first."
    if alive_chunkservers == 3:
        return "Cluster is healthy. All chunkservers are online and replication is available."
    if alive_chunkservers == 2:
        return "Warning: One chunkserver is offline. Restart the failed node to restore full replication."
    if alive_chunkservers == 1:
        return "High risk: Only one chunkserver is online. Data availability may be affected."
    return "CRITICAL: No chunkservers are online. File storage is unavailable."
